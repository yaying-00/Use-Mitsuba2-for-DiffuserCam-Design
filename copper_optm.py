...
def plot_contour(para, title, sigma = 0):
	vertex_pos = ravel(para)
...
def make_scene(integrator, spp, w=256, h=256):
	...
	<scene version="2.1.0">
		...
		<shape type="sphere" id="sphere_emitter">
			...
			<point name="center" x="0" y="0" z="30"/>
		...
		<!-- Scattering model of the copper panel -->
		<bsdf type="roughconductor"   id = "face">
			<string name="distribution" value="beckmann"/>
			<float name="alpha_u" value="0.15"/>
			<float name="alpha_v" value="0.15"/>
...
apply_displacement(disp_tex, amp)
...
# Plot the initial height map of the diffuser
plot_contour(params['grid_mesh.vertex_positions_buf'], title = 'diffuer_ref')
...
plot_contour(params[vertex_pos_key], title = 'diffuer_optm', sigma = 1.0)
print("DONE.")
